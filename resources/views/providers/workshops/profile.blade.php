...
.edit-icon i {
  font-size: 17px;
    ...
    <div class="col-lg-8 profile-right">
        ...
        <form action="{{ route('workshop.update', auth()->user()->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
    ...
    <input 
        ...
        id="workshop_name" 
        class="form-control" 
        value="{{ auth()->user()->workshop_provider->workshop_name ?? '' }}" 
    >
</div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  saveBtn.addEventListener('click', function() {
    ...
    fetch(`/workshop/${id}/update-image`, {
      method: 'POST',
      body: formData
    })
    .then(res => res.json())
